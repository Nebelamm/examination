<template>
  <div class="box">
    ...
    <div class="right">
      <h4>注册</h4>
      <form action="">
        <input class="acc" v-model="users.user" name="user" type="text" placeholder="用户名" autocomplete="off"
               required>
        <input class="acc" v-model="users.password" name="pw" type="password" placeholder="密码" autocomplete="off"
               required>
        <input class="submit" type="submit" value="Reg" v-on:click.prevent="loging">
      </form>
      <div class="fn">
...
export default {
  ...
  data() {
    return {
      users: {user: null, password: null}
    }
  },
  methods: {
    loging() {
      if (this.users.user == 123 && this.users.password == 123) {
